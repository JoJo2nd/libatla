...
atAtlaDataBlob_t* ATLA_API atOpenAtlaDataBlob(atAtlaContext_t* ctx,
  ...
  if (flags & ATLA_WRITE) {
    ...
    totalsize =
      datablob->header_.dataStartOffset_ - datablob->header_.headerSize_;
    ...
    datablob->deserialiseInfo_.objects_ =
      (atSerialisedTOC_t*)(((atuint8*)tmpmem) +
                           (datablob->header_.tocOffset_ -
                            datablob->header_.headerSize_));
    datablob->deserialiseInfo_.objectCount_ =
      (datablob->header_.dataStartOffset_ - datablob->header_.tocOffset_) /
      datablob->header_.tocSize_;
    datablob->deserialiseInfo_.schema_ =
      (atSerialisedSchema_t*)(((atuint8*)tmpmem) +
                              (datablob->header_.schemaOffset_ -
                               datablob->header_.headerSize_));
    ...
    while (tmpmem < tempend) {
      ...
      // TODO: validate that the elements are in offset order, make
      // serialisation simpler
...
atuint ATLA_API atSerialiseDataBlob(atAtlaDataBlob_t* blob) {
  ...
  for (tdbp = blob->dataBlobsHead_, currentObjectID = 0; tdbp;
       tdbp = tdbp->next_, ++currentObjectID) {
  ...
  for (tdbp = blob->dataBlobsHead_, currentObjectID = 0; tdbp;
    ...
    if (tdbp->schema_) {
      ...
      for (elementIdx = 0; elementIdx < tdbp->elementCount_; ++elementIdx) {
        for (typeeleIdx = 0; typeeleIdx < typeSchema->elementCount_;
             ++typeeleIdx) {
          eleDesc = typeSchema->elementArray_ + typeeleIdx;
          eledataptr = rootdataptr + eleDesc->offset_;
          if (eleDesc->nestedID_ != 0) {
            blob->statusCode_ = atSerialiseNestedSchema(
              blob, eleDesc->nestedID_, eleDesc->arrayCount_, eledataptr, ios);
            if (blob->statusCode_ != ATLA_EOK) return blob->statusCode_;
          } else {
            ATLA_IOWRITE(
              ios, eledataptr, eleDesc->size_ * eleDesc->arrayCount_);
          }
        }
...
atErrorCode ATLA_API atSerialiseSchema(atIOAccess_t*   io,
                                       atDataSchema_t* typedesc) {
  atuint                      i;
...
atErrorCode ATLA_API atAddDataToBlob(atAtlaDataBlob_t* blob,
                                     const atchar*     objectname,
                                     const atchar*     schemaname,
                                     atuint32          count,
                                     void*             data) {
  ...
  for (tdbp = blob->dataBlobsHead_; tdbp; tdbp = tdbp->next_) {
    ...
    if (data >= tdbp->pointer_ &&
        data < (void*)(((atuint8*)tdbp->pointer_) +
                       (schema->typeSize_ * tdbp->elementCount_)))
      return ATLA_DUPLICATE_DATA;
  }

  newtdbp = blob->memHandler_.memAlloc_(sizeof(atDataBlob_t),
                                        blob->memHandler_.memUser_);
...
atErrorCode ATLA_API atAddTypelessDataToBlob(atAtlaDataBlob_t* blob,
                                             const atchar*     objectname,
                                             atuint32          size,
                                             atuint32          count,
                                             void*             data) {
  ...
  for (tdbp = blob->dataBlobsHead_; tdbp; tdbp = tdbp->next_) {
    ...
    // In the future, Atla may handle this in some clever way
    if (data >= tdbp->pointer_ &&
        data < (void*)(((atuint8*)tdbp->pointer_) + (size * count)))
      return ATLA_DUPLICATE_DATA;
  }
...
void ATLA_API atInitDataBlob(atAtlaDataBlob_t*  datablob,
...
atErrorCode ATLA_API atAddSchemaToBlob(atAtlaDataBlob_t* ctx,
                                       atDataSchema_t*   schema) {
  ...
  newutdp = ctx->memHandler_.memAlloc_(sizeof(atUsedSchema_t),
                                       ctx->memHandler_.memUser_);
...
atErrorCode ATLA_API atSerialiseNestedSchema(atAtlaDataBlob_t* blob,
                                             atUUID_t          schemaID,
                                             atuint32          count,
                                             atuint8*          rootdataptr,
                                             atIOAccess_t*     iostream) {
  atuint             typeeleIdx;
  atuint             rootele;
  atDataSchema_t*    schema;
  ...
  for (rootele = 0; rootele < count; ++rootele) {
    for (typeeleIdx = 0; typeeleIdx < schema->elementCount_; ++typeeleIdx) {
      ...
      if (eleDesc->nestedID_ != 0) {
        blob->statusCode_ = atSerialiseNestedSchema(
          blob, eleDesc->nestedID_, eleDesc->arrayCount_, eledataptr, iostream);
        if (blob->statusCode_ != ATLA_EOK) return blob->statusCode_;
      } else {
        ATLA_IOWRITE(
          iostream, eledataptr, eleDesc->size_ * eleDesc->arrayCount_);
      }
    }
...
atErrorCode ATLA_API
atGetSerialisedSchema(atAtlaDataBlob_t*             ctx,
                      atUUID_t                      schemaID,
                      atSerialisedSchema_t**        outschema,
                      atSerialisedSchemaElement_t** outeles) {
  ...
  for (i = 0; i < imax; ++i) {
    ...
    schema = (atSerialisedSchema_t*)(((atuint8*)schema) +
                                     sizeof(atSerialisedSchema_t) +
                                     (schema->elementCount_ *
                                      sizeof(atSerialisedSchemaElement_t)));
  }

  return ATLA_EBADSCHEMAID;
}

void* atGetOutputOffset(atDataSchema_t* schema,
                        atUUID_t        id,
                        void*           ptr,
                        atUUID_t*       typeID) {
  atuint i;
...
atErrorCode ATLA_API atResolveAndDeserialseData(atAtlaDataBlob_t* blob,
                                                atUUID_t          typeID,
                                                atuint            count,
                                                void*             outptr) {
  /*
  ...
  blob->statusCode_ =
    atGetSerialisedSchema(blob, typeID, &dataSchema, &dataSchemaElements);
  if (blob->statusCode_ != ATLA_EOK) return blob->statusCode_;
  ...
  for (object = 0; object < count; ++object) {
    for (dataEle = 0; dataEle < dataEleCount; ++dataEle) {
      datasize = dataSchemaElements[dataEle].size_ *
                 dataSchemaElements[dataEle].arraycount_;
      // find correct offset into outptr
      eleptr = atGetOutputOffset(
        schema, dataSchemaElements[dataEle].elementID_, rootptr, &schemaTypeID);
      ...
      if (eleptr) {
        ...
        if (dataSchemaElements[dataEle].typeID_ == 0) {
          ...
          blob->statusCode_ =
            atResolveAndDeserialseData(blob,
                                       schemaTypeID,
                                       dataSchemaElements[dataEle].arraycount_,
                                       eleptr);
...
atErrorCode ATLA_API atGetDataDescByIndex(atAtlaDataBlob_t* blob,
                                          atuint            idx,
                                          atDataDesc_t*     outdesc) {
...
atErrorCode ATLA_API atGetDataDescByName(atAtlaDataBlob_t* blob,
                                         const atchar*     objectname,
                                         atDataDesc_t*     outdesc) {
  atuint   i, iend;
  atUUID_t id;
...
atErrorCode ATLA_API atDeserialiseDataByIndex(atAtlaDataBlob_t* blob,
                                              atuint            idx,
                                              void*             output) {
  atUUID_t typeID;
  atuint   count;
  ...
  ATLA_IOSEEK(&blob->iostream_,
              blob->deserialiseInfo_.objects_[idx].fileoffset_,
              eSeekOffset_Begin);
  if (typeID != 0) {
    ...
    ATLA_IOREAD(&blob->iostream_,
                output,
                count * blob->deserialiseInfo_.objects_[idx].size_);
  }

  return ATLA_EOK;
}

atErrorCode ATLA_API atDeserialiseDataByName(atAtlaDataBlob_t* blob,
                                             const atchar*     objectname,
                                             void*             output) {
  atuint   i, iend;
  atUUID_t id;
